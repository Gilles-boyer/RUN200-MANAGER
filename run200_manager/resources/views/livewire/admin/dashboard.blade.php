    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <x-racing.card>
            <div class="p-6">
                ...
                @php($carCategoryStats = $this->carsByCategory)
                @if(count($carCategoryStats['labels']) > 0)
                    <p class="text-sm text-carbon-400 mb-3">
                        {{ $carCategoryStats['total'] ?? array_sum($carCategoryStats['data']) }} voitures dans {{ $carCategoryStats['category_count'] ?? count($carCategoryStats['labels']) }} catégories.
                        @if(($carCategoryStats['category_count'] ?? count($carCategoryStats['labels'])) > 8)
                    ...
                    <x-racing.chart
                        ...
                        type="doughnut"
                        height="280px"
                        :labels="$carCategoryStats['labels']"
                        :datasets="$carCategoryStats['data']"
                    ...
                    <div class="mt-4 max-h-64 overflow-y-auto divide-y divide-carbon-700/50" aria-label="Détail des voitures par catégorie">
                        @foreach(($carCategoryStats['breakdown'] ?? []) as $category)
